...
%Each character will take 8 bits so total number of bits in the message will be len
len=strlength(message)*8;

%ascii is a vector having the ascii value of each character
ascii=uint8(message);

%binary_separate is an array having the decimal representation of each ascii value
binary_separate=dec2bin(ascii,8);  

%binary_all will have the entire sequence of bits of the message
binary_all='';  
